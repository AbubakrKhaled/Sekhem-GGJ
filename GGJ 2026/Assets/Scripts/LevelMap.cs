...
//    [Header("Grid Offset")]
//    public int gridOffsetX = 14;
//    public int gridOffsetY = -29;
...
//        int x = cell.x - gridOffsetX;
//        int y = cell.y - gridOffsetY;
...
//    //    Debug.Log($"Player grid: {grid.x},{grid.y} | Checking floor {floor + 1}");
...
//        Vector2Int center = GetGrid(worldPos);

//        for (int y = -1; y <= 1; y++)
//            for (int x = -1; x <= 1; x++)
//            {
//                if (HasGround(floor + 1, center.x + x, center.y + y))
//                    return true;
//            }
...
public class LevelMap : MonoBehaviour
{
    ...
    [Header("Logic Tilemaps (index = floor)")]
    public Tilemap[] logicTilemaps; // size = number of floors

    [Header("Reference Tilemap (for grid conversion)")]
    ...
    {
        Instance = this;
        Debug.Log($"Logic tilemaps count: {logicTilemaps.Length}");
    ...
    public bool HasGround(int floor, int x, int y)
    {
        if (floor < 0 || floor >= logicTilemaps.Length)
            return false;

        Tilemap tm = logicTilemaps[floor];
    ...
    public bool CanJumpUp(int floor, Vector3 worldPos)
    {
        if (floor >= logicTilemaps.Length - 1)
            return false;
